<template>
  ...
  <el-table ref="multipleTableRef" :data="commodityPageList" @selection-change="handleSelectionChange" style="width: 100%; margin-top: 10px;" table-layout="auto" size="large" border stripe>
    ...
    <el-table-column label="操作" fixed="right" width="240">
      <template #default="props">
        ...
        <el-link type="primary" @click.prevent="openInstoreAdd(props.row)">入库</el-link>
  ...
  <!-- 添加采购单对话框 -->
  <instore-add ref="instoreAddRef" @ok="toInstoreList"></instore-add>
...
const getCommodityPageList = () => {
  get("/product/product", params).then(result => {
...
// 获取所有供应商
const getSupplyList = () => {
  get("/product/supply").then(result => {
...
// 跳向添加入库单
import InstoreAdd from "../../instore/instore-add.vue";
const instoreAddRef = ref();
const openInstoreAdd = (commodity) => {
  instoreAddRef.value.open(commodity);
};

// 跳转到入库单列表页面
const toInstoreList = (storeId) => {
  // 通过路由导航到入库单列表页面
  router.push({ path: "/instore/index", query: {"storeId": storeId } });
}
...
const changState = (productId, upDownState) => {
  put(`/product/state`, {productId, upDownState}).then(result => {
    tip.success(result.message);
    // 重新查询
    getCommodityPageList();
  });
